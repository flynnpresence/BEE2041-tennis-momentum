...
Figure 2 tracks the Cumulative Sum (CUSUM) score across the longest WTA match, a running tally of overperformance. When a player wins points faster than their match average, the line climbs. Genuine momentum would look like sustained directional movement.
...
Instead, Alexandrova performs above her average almost continuously. The line stays positive for 88 points, dips briefly, then sharply recovers to finish above +4. Du et al. (2025) would interpret this oscillation as shifting momentum. It is simply a player performing consistently well, interrupted by a single brief slump. That looks less like momentum and more like ordinary match variance. This single match is illustrative, not a formal test.
...
Figure 6 opens with the combined effect; clicking "Split by Pressure Type" reveals the break-point and tiebreak estimates separately. Winning a break point boosts the probability of winning the next point by around 15 percentage points in the ATP and 6 in the WTA. But this positive signal is not evidence of psychological momentum. The evidence points instead to a structural feature of how tennis scoring works.
...
In tiebreaks, where the serving advantage is largely neutralised by alternating serve structure, the ATP momentum signal disappears. Serve alternates every two points, so winning a tiebreak point confers no clear advantage on the next. In the ATP, the estimated effect is statistically indistinguishable from zero (+0.0003, SE = 0.0016). In the WTA, the estimated effect is negative (-0.0564, SE = 0.0086); the WTA sample is smaller (209 treated observations) and the magnitude should be treated with caution, but the interval excludes zero entirely.
