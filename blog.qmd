...
In tiebreaks, where the serving advantage is largely neutralised by alternating serve structure, the ATP momentum signal disappears. Serve alternates every two points, so winning a tiebreak point confers no clear advantage on the next. In the ATP, the estimated effect is statistically indistinguishable from zero (+0.0003, SE = 0.0016). In the WTA, the estimated effect is negative (-0.0564, SE = 0.0086); the WTA sample is smaller (209 treated observations), the magnitude should be treated with caution, and the standard error is a CLT approximation: individual CATE estimates are not independent draws, so the boundary should be read as indicative rather than definitive.
