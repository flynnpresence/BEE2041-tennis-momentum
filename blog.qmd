...
The combined estimated effect is positive for both tours (ATP: +0.0888, WTA: +0.0550). If I stopped here, the picture would suggest that winning any high-leverage moment provides a consistent positive boost. But this combined number treats break points and tiebreaks as identical. What happens when I split them apart?
...
Figure 6 opens with the combined effect; clicking "Split by Pressure Type" reveals the break-point and tiebreak estimates separately. The next-point win probability following a won break point is around 14 percentage points higher in the ATP and 7 in the WTA, holding skill, form, and momentum constant. But this positive signal is not evidence of psychological momentum. The evidence points instead to a structural feature of how tennis scoring works.
...
In tiebreaks, where the serving advantage is largely neutralised by alternating serve structure, the momentum signal disappears on both tours. Serve alternates every two points, so winning a tiebreak point confers no clear advantage on the next. In the ATP, the estimated effect is statistically indistinguishable from zero (+0.0072, 95% CI [-0.023, 0.044]). In the WTA, despite a smaller sample (209 treated observations), the estimated effect is also indistinguishable from zero (+0.0024, 95% CI [-0.085, 0.066]). Both intervals come from a bootstrap that resamples whole matches rather than individual points, since points within the same match are not independent observations.

The case for a structural rather than psychological explanation does not rest on either tiebreak estimate landing at exactly zero. It rests on the gap: a break-point effect around fourteen percentage points in the ATP against a tiebreak effect neither tour can distinguish from zero. A boost tied to the serving structure should appear where that structure creates an advantage and disappear where it doesn't; that is the pattern in the data, and it holds regardless of exactly where either tiebreak estimate happens to land within its interval.
...
The cumulative match tracker (CUSUM) dominates. Accumulated match drift (what CUSUM captures: the cumulative gap between actual and average performance) accounts for around 53% of the model's weight in the ATP and 52% in the WTA, with baseline player rank and rolling win percentage accounting for most of the remainder.
