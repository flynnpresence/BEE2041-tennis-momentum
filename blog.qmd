...
The combined estimated effect is positive for both tours (ATP: +0.0844, WTA: +0.0478). If I stopped here, the picture would suggest that winning any high-leverage moment provides a consistent positive boost. But this combined number treats break points and tiebreaks as identical. What happens when I split them apart?
...
Figure 6 opens with the combined effect; clicking "Split by Pressure Type" reveals the break-point and tiebreak estimates separately. The next-point win probability following a won break point is around 13 percentage points higher in the ATP and 6 in the WTA, holding skill, form, and momentum constant. But this positive signal is not evidence of psychological momentum. The evidence points instead to a structural feature of how tennis scoring works.
...
In tiebreaks, where the serving advantage is largely neutralised by alternating serve structure, the momentum signal disappears on both tours. Serve alternates every two points, so winning a tiebreak point confers no clear advantage on the next. In the ATP, the estimated effect is statistically indistinguishable from zero (-0.0066, 95% CI [-0.029, 0.047]). In the WTA, despite a smaller sample (209 treated observations), the estimated effect is also indistinguishable from zero (-0.0091, 95% CI [-0.091, 0.074]). Both intervals come from a bootstrap that resamples whole matches rather than individual points, since points within the same match are not independent observations.

The case for a structural rather than psychological explanation does not rest on either tiebreak estimate landing at exactly zero. It rests on the gap: a break-point effect around thirteen percentage points in the ATP against a tiebreak effect neither tour can distinguish from zero. A boost tied to the serving structure should appear where that structure creates an advantage and disappear where it doesn't; that is the pattern in the data, and it holds regardless of exactly where either tiebreak estimate happens to land within its interval.
...
There's a sharper version of this test: server game points are the mirror image of break points — same high stakes, but winning one hands the *next* serve to the opponent instead of you. Winning a break point raises next-point win probability; winning a server game point lowers it, on both tours. The sign flips with the serve — not with who won the point — which is what you'd expect if the effect is really about the serve, and not about momentum.

## If Not Momentum, Then What?

The serving mechanism explains whether the effect appears; it doesn't explain how large it gets across different players and situations — that's a separate question, and Figure 7 ranks the model's control variables by their predictive weight.
...
The cumulative match tracker (CUSUM) dominates. Accumulated match drift (what CUSUM captures: the cumulative gap between actual and average performance) accounts for around 54% of the model's weight in the ATP and 58% in the WTA, with baseline player rank and rolling win percentage accounting for most of the remainder.

Crucially, Winning Streak (whether a player won their last four consecutive points) scores a virtually flat 1% in the ATP and 2% in the WTA. This means that whether a player is on a hot streak does not meaningfully change how large the estimated effect of a break-point win is, a distinct question from whether streaks predict outcomes in general, which the independence tests above address directly. Within the causal forest, streak status adds almost nothing once match drift is accounted for.
