...
(function () {
  ...
  function initFeatureImportanceChart() {
    ...
    const margin = { top: 24, right: 72, bottom: 44, left: 220 };
    const width = container.clientWidth || 900;
    const rowHeight = 44;
    const height = featureData.length * rowHeight + margin.top + margin.bottom;
    const innerWidth = width - margin.left - margin.right;
    const innerHeight = height - margin.top - margin.bottom;
    ...
    const maxVal = d3.max(featureData, (d) => Math.max(d.ATP, d.WTA));
    ...
    xAxis.selectAll("line").attr("stroke", "#D9D4C5");
    xAxis.selectAll("path").attr("stroke", "#D9D4C5");
    xAxis
      .selectAll("text")
      .style("font-family", "IBM Plex Mono, monospace")
      .style("font-size", "10.5px")
    ...
    g.selectAll(".bar-wta")
      ...
      .attr("class", "bar-wta")
      .attr("x", 0)
      .attr("y", (d) => y(d.feature) + barHeight + 4)
      .attr("width", 0)
      .attr("height", barHeight)
      .attr("fill", "#B25B2E");

    const atpValues = g.selectAll(".val-atp")
      .data(featureData)
      .join("text")
      .attr("class", "val-atp")
      .attr("y", (d) => y(d.feature) + barHeight / 2)
      .attr("dy", "0.35em")
      .style("font-family", "IBM Plex Mono, monospace")
      .style("font-size", "10.5px")
      .style("fill", "#2A4A6B")
    ...
    const wtaValues = g.selectAll(".val-wta")
      .data(featureData)
      .join("text")
      .attr("class", "val-wta")
      .attr("y", (d) => y(d.feature) + barHeight + 4 + barHeight / 2)
      .attr("dy", "0.35em")
      .style("font-family", "IBM Plex Mono, monospace")
      .style("font-size", "10.5px")
      .style("fill", "#B25B2E")
    ...
    const legend = g.append("g").attr("transform", `translate(${innerWidth - 160}, -14)`);

    legend.append("rect").attr("width", 12).attr("height", 12).attr("fill", "#2A4A6B");
    legend.append("text")
      .attr("x", 18)
      .attr("y", 10)
      .style("font-family", "IBM Plex Sans, sans-serif")
    ...
    legend.append("rect").attr("x", 70).attr("width", 12).attr("height", 12).attr("fill", "#B25B2E");
    legend.append("text")
      .attr("x", 88)
      .attr("y", 10)
      .style("font-family", "IBM Plex Sans, sans-serif")
